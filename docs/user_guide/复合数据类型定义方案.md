...
本文档基于`g:\04 rust\PDSD-v0.2\src\editor\business\text_circuit.rs`中已定义的参数，详细定义了PDSD系统中使用的复合数据类型，特别是回路数据（CircuitData）和配电箱数据（DistributionBoxData）的结构和实现方案，确保参数定义与现有代码保持一致。
...
impl CircuitPurpose {
    pub fn to_str(&self) -> &'static str {
        match self {
            CircuitPurpose::Lighting => "照明",
            CircuitPurpose::Power => "动力",
            CircuitPurpose::Socket => "插座",
            CircuitPurpose::AirConditioning => "空调",
            CircuitPurpose::SpecialEquipment => "特殊设备",
        }
    }
    
    pub fn get_default_kx(&self) -> f64 {
        match self {
            CircuitPurpose::Lighting => 0.8,
            CircuitPurpose::Power => 0.7,
            CircuitPurpose::Socket => 0.5,
            CircuitPurpose::AirConditioning => 0.8,
            CircuitPurpose::SpecialEquipment => 0.9,
        }
    }
    
    pub fn get_default_cos(&self) -> f64 {
        match self {
            CircuitPurpose::Lighting => 0.9,
            CircuitPurpose::Power => 0.75,
            CircuitPurpose::Socket => 0.8,
            CircuitPurpose::AirConditioning => 0.85,
            CircuitPurpose::SpecialEquipment => 0.8,
        }
    }
}
...
impl Default for CircuitData {
    fn default() -> Self {
        let default_number = CircuitNumber::new(1);
        Self {
            circuit_id: default_number.to_str(),
...
## 三、配电箱数据复合类型 (DistributionBoxData)
    ...
    fn default() -> Self {
        Self {
            ...
            incoming_components: vec![
                ElectricComponent::Breaker(Breaker {
...
impl DistributionBoxData {
    ...
    pub fn assign_circuit_ids(&mut self) {
        ...
        // 按输出端口连接顺序从1开始分配WL格式的编号
        for (i, circuit) in circuits_data.iter().enumerate() {
            let circuit_number = CircuitNumber::new((i + 1) as u32);
            let new_id = circuit_number.to_str();
    ...
    pub fn add_circuit(&mut self, mut circuit: CircuitData) {
        // 添加回路，使用WL格式的编号
        let circuit_number = CircuitNumber::new((self.circuits.len() + 1) as u32);
        let circuit_id = circuit_number.to_str();
    ...
    /// 获取回路的断路器参数
    pub fn get_circuit_breaker(&self, circuit_id: &str) -> Option<&Breaker> {
        self.circuits.get(circuit_id).and_then(|circuit| {
            circuit.components.iter()
                .find_map(|component| {
                    if let ElectricComponent::Breaker(breaker) = component {
                        Some(breaker)
                    } else {
                        None
                    }
    ...
    /// 获取进线断路器参数
    pub fn get_incoming_breaker(&self) -> Option<&Breaker> {
    ...
    /// 查找进线断路器
    pub fn find_main_breaker(&self) -> Option<&Breaker> {
    ...
    /// 更新进线断路器
    pub fn update_main_breaker(&mut self, breaker: Breaker) -> bool {
...
6. **一致性验证**：所有枚举类型（VoltageLevel、LayingMethod、PipeSpecification、LayingArea、PhaseSequence、PhaseLine、TwinParallelCables、PE、CoresNum等）的定义和实现均与`text_circuit.rs`保持一致，确保系统各部分参数定义统一。